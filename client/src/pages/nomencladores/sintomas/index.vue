<template>
  <q-page class="q-pa-sm">
    <q-table
      ...
      row-key="id"
      @request="getSintomas"
      no-data-label="No existen sintomas registradas"
      binary-state-sort
    >
      <template v-slot:top = props>
        <q-toolbar class="q-pb-md">
          <q-btn flat color="cyan-9" round dense icon="pending_actions" />
          <q-toolbar-title>Sintoma</q-toolbar-title>
          <q-btn
            flat
            round
            dense
...
import AddupdSintomas from 'src/forms/sintomas/addupdSintomas.vue';
export default {
  components: { window, confirm, BtnCancel, BtnAceptar, AddupdSintomas},
  name: "SintomasIndex",
  data() {
    return {
      rows: [],
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Nuevo sintoma" : "Modificar sintoma";
    }
  },
  methods: {
    buscar() {
    ...
    async getSintomas(props) {
      const { page, rowsPerPage, filter, descending } = props.pagination;
      const response = await this.$axios.get("sintoma", {
        params: {
    ...
    async deleteItem(item) {
      ...
      if (
        await this.$refs.confirm.open(
          "Eliminar",
          "Usted está seguro que desea eliminar el sintoma?",
          { color: "red" }
        )
      ) {
        const result = await this.$axios.delete(
          `sintoma/${this.editedItem.id}`
        );
        if (result.status === 200) {
          this.$q.notify({
            ...
            textColor: "white",
            icon: "cloud_done",
            message: "Sintoma eliminado satisfactorimente."
          });
    ...
    fireEvent() {
      const formHasError = !this.$refs.form.$refs.descripcion.validate()
        ? true
        : false;
      ...
      if (!formHasError) {
        if (this.editedIndex === -1) {
          ...
          this.$axios
            .patch(`sintoma/${this.editedItem.id}`, this.editedItem)
            .then(response => {
              this.getSintomas({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Sintoma modificado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
