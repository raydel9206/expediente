<template>
  <q-page class="q-pa-sm">
    <q-table
      ...
      no-data-label="No existen antecedentes registradas"
      binary-state-sort
    >
      <template v-slot:top =  props>
      <q-toolbar  class="q-pb-md">
        <q-btn flat color="cyan-9" round dense icon="format_list_numbered" />
          <q-toolbar-title>Antecedente</q-toolbar-title>
          <q-btn
            flat
            round
            dense
...
import AddupdAntecedentes from 'src/forms/antecedentes/addupdAntecedentes.vue';

export default {
  components: { window, confirm, BtnCancel, BtnAceptar, AddupdAntecedentes},
  name: "AntecedentesIndex",
  data() {
    return {
      rows: [],
      confirmTitle: "",
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1
        ? "Nuevo antecedente patologico"
        : "Modificar antecedente patologico";
    }
  },
  methods: {
    buscar() {
    ...
    async getAntecedentes(props) {
      const { page, rowsPerPage, filter, descending } = props.pagination;
      const response = await this.$axios.get("antecedente", {
        params: {
    ...
    async deleteItem(item) {
      ...
      if (
        await this.$refs.confirm.open(
          "Eliminar",
          "Usted está seguro que desea eliminar el antecedente patologico?",
          { color: "red" }
        )
      ) {
        const result = await this.$axios.delete(
          `antecedente/${this.editedItem.id}`
        );
        if (result.status === 200) {
          this.$q.notify({
            ...
            textColor: "white",
            icon: "cloud_done",
            message: "Antecedente eliminado satisfactorimente."
          });
    ...
    close() {
      ...
      this.$refs.form.$refs.nombre.resetValidation()
       this.$refs.form.$refs.descripcion.resetValidation()
    },

    fireEvent() {
      const formHasError =
        !this.$refs.form.$refs.nombre.validate() ||
        !this.$refs.form.$refs.descripcion.validate()
          ? true
          : false;
      ...
      if (!formHasError) {
        if (this.editedIndex === -1) {
          this.$axios
            .post("antecedente", this.editedItem)
            .then(response => {
              this.getAntecedentes({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Antecedente adicionado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
          ...
          this.$axios
            .patch(`antecedente/${this.editedItem.id}`, this.editedItem)
            .then(response => {
              this.getAntecedentes({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Antecedente modificado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
