<template>
  <q-page class="q-pa-sm">
    <q-table
      ...
      row-key="id"
      @request="getPoliclinico"
      no-data-label="No existen policlínicos registrados"
      binary-state-sort
    >
      <template v-slot:top = props>
       <q-toolbar  class="q-pb-md">
           <q-btn flat color="cyan-9" round dense icon="health_and_safety" />
          <q-toolbar-title>Policlínico</q-toolbar-title>
          <q-btn
            flat
            round
            dense
...
import AddupdPoliclinico from 'src/forms/policlinico/addupdPoliclinico.vue';

export default {
  components: { window, confirm, BtnCancel, BtnAceptar, AddupdPoliclinico },
  name: "PoliclinicoIndex",
  data() {
    return {
      rows: [],
      confirmTitle: "",
  ...
  methods: {
    ...
    async getPoliclinico(props) {
      const { page, rowsPerPage, filter, descending } = props.pagination;
      const response = await this.$axios.get("policlinico", {
        params: {
    ...
    async deleteItem(item) {
      ...
      if (
        await this.$refs.confirm.open(
          "Eliminar",
          "Usted está seguro que desea eliminar el policlinico?",
          { color: "red" }
        )
      ) {
        const result = await this.$axios.delete(
          `policlinico/${this.editedItem.id}`
        );
        if (result.status === 200) {
          this.$q.notify({
            ...
            textColor: "white",
            icon: "cloud_done",
            message: "Policlinico eliminado satisfactorimente."
          });
    ...
    fireEvent() {
      ...
      if (!formHasError) {
        if (this.editedIndex === -1) {
         this.$axios
            .post("policlinico", this.editedItem)
            .then(response => {
              this.getPoliclinico({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Policlinico adicionado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
           ...
           this.$axios
            .patch(`policlinico/${this.editedItem.id}`, this.editedItem)
            .then(response => {
              this.getPoliclinico({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Policlinico modificado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
