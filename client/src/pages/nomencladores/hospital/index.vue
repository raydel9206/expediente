<template>
  <q-page class="q-pa-sm">
    <q-table
      ...
      row-key="id"
      @request="getHospital"
      no-data-label="No existen hospitales registradas"
      binary-state-sort
    >
      <template v-slot:top="props">
        <q-toolbar class="q-pb-md">
          <q-btn flat color="cyan-9" round dense icon="local_hospital" />
          <q-toolbar-title>Hospital</q-toolbar-title>
          <q-btn
            flat
            round
            dense
...
import AddupdHospital from 'src/forms/hospital/addupdHospital.vue';

export default {
  components: { window, confirm, BtnCancel, BtnAceptar, AddupdHospital },
  name: "HospitalIndex",
  data() {
    return {
      rows: [],
  ...
  methods: {
    ...
    async deleteItem(item) {
      ...
      if (
        await this.$refs.confirm.open(
          "Eliminar",
          "Usted está seguro que desea eliminar el hospital?",
          { color: "red" }
        )
      ) {
        const result = await this.$axios.delete(
          `hospital/${this.editedItem.id}`
        );
        if (result.status === 200) {
          this.$q.notify({
            ...
            textColor: "white",
            icon: "cloud_done",
            message: "Hospital eliminado satisfactorimente."
          });
    ...
    fireEvent() {
      ...
      if (!formHasError) {
        if (this.editedIndex === -1) {
          this.$axios
            ...
            .then(response => {
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Hospital adicionado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
          ...
          this.$axios
            .patch(`hospital/${this.editedItem.id}`, this.editedItem)
            .then(response => {
              this.getHospital({
                pagination: this.pagination
              });
              ...
              this.$q.notify({
                ...
                textColor: "white",
                icon: "cloud_done",
                message: "Hospital modificado satisfactorimente."
              });
            })
            .catch(error => {
              this.$q.notify({
    ...
    async loadMunicipio() {
      const response = await this.$axios.get("municipio/byProvincia", {
        params: { provincia_id: this.editedItem.provincia.id }
      });
      if (response.status === 200) {
